<div class="divisor1">
	<p class="text-right noPrint"> <a href="#" onclick="window.print()" class="btn_search radius" title="Imprimir este relatório"><i class="fa fa-print"></i></a></p>
	<h1 class="font-text-min font-weight-medium"  style="margin: 10px 0 !important;">Relatório de Acessos</h1>
	<table style="width: 96% !important; margin: 10px 2% !important;">
		
		<?php for($i = 0; $i < 5; $i++): ?>
		<tr>
			<td>
				<p class="font-text-sub"><b>Usuário:</b></p>
				<p>Jeferson Souza</p>
			</td>
			
			<td>
				<p class="font-text-sub"><b>Data Entrada:</b></p>
				<p>20/12/2022 - 12:56</p>
			</td>
			
			<td>
				<p class="font-text-sub"><b>Data Saída:</b></p>
				<p>20/12/2022 - 17:24</p>
			</td>
			
		</tr>
		
		<?php endfor; ?>
	</table>
</div>